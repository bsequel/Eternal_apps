$(document).ready(function () {
    ...
    function toggleCamsAutoEtl() {
        ...
        if (camsEtlAutoInterval) {
            ...
            showToast('CAMS Auto ETL enabled - Running every 24 minutes', 'success');
        }
    }


    // function startCamsAutoEtl() {
    //     let camsTimeLeft = 86399;
    ...
    function startCamsAutoEtl() {
    const camsCountdownDiv = $('#camsEtlCountdown');
    let remaining = 24 * 60 * 60; // 24 hours in seconds (86400)
    ...
    camsEtlAutoInterval = setInterval(() => {
        remaining--;
        ...
        camsCountdownDiv.text(
            `Next run in ${hrs}h ${mins}m ${secs}s`
        );

        // When countdown ends, trigger ETL and reset
        if (remaining <= 0) {
            remaining = 24 * 60 * 60; // reset to 24 hours
            runCamsEtl(); // <-- your ETL function
        }
    ...
    function initCaseTable() {
            ...
            columnDefs: [
                ...
                // },
                {
                    ...
                    render: function (data) {
                        if (!data) return `<span class="text-muted">No PDF</span>`;
                        return `
                                    <a href="${data}" download class="btn btn-sm btn-outline-primary">
        ...
        $(document).on('change', '.lien-input', function () {
            ...
            //     refreshTableData();
            // }
            simpleConfirm(`Update lien amount to "${newValue}"?`, function (confirmed) {
                if (confirmed) {
                    sendCaseUpdate(caseId, { lien_amount: newValue });
                } else {
                    refreshTableData();
        ...
        $(document).on('change', '.remarks-input', function () {
            ...
            // }
            simpleConfirm(`Update remarks to "${newValue}"?`, function (confirmed) {
                if (confirmed) {
                    sendCaseUpdate(caseId, { remarks: newValue });
                } else {
                    $('#casesTable').DataTable().ajax.reload(null, false);
        ...
        $(document).on('change', '.utr-input', function () {
            ...
            //     refreshTableData();
            // }
            simpleConfirm(`Update UTR Number to "${newValue}"?`, function (confirmed) {
                if (confirmed) {
                    sendCaseUpdate(caseId, { utr_number: newValue });
                } else {
                    refreshTableData();
    ...
    function initUserTable() {
        ...
        $('#userTable').on('change', '.role-selector', function () {
            ...
            const newRole = $(this).val();
            const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]').value;
            const dropdown = $(this); // Store reference for use in callback

            // Confirm before sending request
            simpleConfirm(`Are you sure you want to change this user's role to "${newRole}"?`, function (confirmed) {
                if (!confirmed) {
                    // ❌ If user cancels, reset select to previous value
                    userTable.ajax.reload(null, false);
                    return;
                ...
                fetch(`/update-user-role/${userId}/`, {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            // update row instantly
                            const row = userTable.row(dropdown.closest('tr'));
                            const rowData = row.data();
                            rowData.role = newRole;
                            row.data(rowData).draw(false);
...
//         localStorage.setItem('camsAutoEtlOn', 'false');
//         localStorage.removeItem('camsNextRun');
...
//         localStorage.setItem('camsAutoEtlOn', 'true');
//         showToast('CAMS Auto ETL enabled - Running daily at 9 AM', 'success');
//     }
// }
...
//     next.setHours(9, 0, 0, 0); // set to 9:00:00 AM

//     if (now >= next) {
//         // already past 9 AM today → schedule tomorrow
...
//     // restore or set next run
//     let nextRun = localStorage.getItem('camsNextRun');
//     if (!nextRun) {
//         nextRun = getNext9AM();
//         localStorage.setItem('camsNextRun', nextRun);
//     } else {
//         nextRun = parseInt(nextRun, 10);
//     }

//     camsEtlAutoInterval = setInterval(() => {
//         let remaining = Math.floor((nextRun - Date.now()) / 1000);

//         if (remaining <= 0) {
//             runCamsEtl(); // <-- call your ETL job

//             // set new run time
//             nextRun = getNext9AM();
//             localStorage.setItem('camsNextRun', nextRun);

//             remaining = Math.floor((nextRun - Date.now()) / 1000);
//         }
...
//         camsCountdownDiv.text(
//             `Next run at 9 AM in ${hrs}h ${mins}m ${secs}s`
//         );
//     }, 1000);
// }
...
// $(document).ready(function () {
//     const camsAutoOn = localStorage.getItem('camsAutoEtlOn') === 'true';
//     if (camsAutoOn) {
//         toggleCamsAutoEtl();
//     }
// });
...
//         localStorage.setItem('camsAutoEtlOn', 'true');
//         showToast('CAMS Auto ETL enabled - Running daily at 8 AM', 'success');
//     }
// }
...
//     next.setHours(8, 0, 0, 0); // set to 8:00:00 AM

//     if (now >= next) {
//         // already past 8 AM today → schedule tomorrow
//         next.setDate(next.getDate() + 1);
//     }
//     return next.getTime();
// }
...
//         if (remaining <= 0) {
//             runCamsEtl(); // <-- your ETL job function
...
//         camsCountdownDiv.text(
//             `Next run at 8 AM in ${hrs}h ${mins}m ${secs}s`
//         );
//     }, 1000);
// }
